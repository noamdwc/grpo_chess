...
from src.grpo_self_play.chess.rewards import cached_raw_reward_white
...
def evaluate_position_quality(board: chess.Board, depth: int = 2) -> Optional[float]:
    ...
    try:
        fen = board.fen()
        eval_cp = cached_raw_reward_white(fen, depth)
        return eval_cp
    except Exception:
        return None
...
class ChessStartStatesDataset(IterableDataset):
  ...
  def __init__(
      ...
      self.phase_distribution = phase_distribution or {"opening": 0.3, "middlegame": 0.5, "endgame": 0.2}
      self.min_eval_cp = min_eval_cp
      self.max_eval_cp = max_eval_cp
      self.use_opening_book = use_opening_book
      self.stockfish_filter_depth = stockfish_filter_depth
      ...
      # Position cache
      self._position_cache: deque = deque(maxlen=cache_size)
